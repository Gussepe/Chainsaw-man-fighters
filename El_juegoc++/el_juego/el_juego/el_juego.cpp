...
int main()
{
	...
	RenderWindow window(VideoMode(1366, 768), "Computer Science's CSM fighters", Style::Close | Style::Resize);
	...
	if (!background.loadFromFile("Images/ecenario1.jpg"))
		return EXIT_FAILURE;
	...
	if (!music.openFromFile("musica2.wav"))
		return EXIT_FAILURE;
	music.setLoop(true);
	music.play();
	SoundBuffer shotBuffer;
	if (!shotBuffer.loadFromFile("shoot.wav")) {
		return EXIT_FAILURE;
	}
	Sound shotSound;
